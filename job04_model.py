...
X_train, X_test, Y_train, Y_test = np.load('./movie_data_218_wordsize_773.npy', allow_pickle=True)
# allow_pickle=True: 피클배열을 읽을 수 있다.
print(X_train.shape, Y_train.shape)
print(X_test.shape, Y_test.shape)

model = Sequential()
model.add(Embedding(773, 300, input_length=218)) # 자연어를 학습하는 layer / 10855개의 차원공간을 만들고 300개(느낌적인 감으로 설정)의 차원공간으로 줄인다
